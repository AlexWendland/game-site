...
def test_completely_random_game(number_of_players: int):
    ...
    for player_index in range(number_of_players):
        ...
        logic.set_player_bid(
            player, random.randint(0, 1), set_suit=random.choice([0, 1, 2, 3]) if game_state.trump_to_be_set else -1
        )
    ...
    for round_number in range(2, rounds + 1):
        ...
        for player_index in range(number_of_players):
            ...
            logic.set_player_bid(
                ...
                set_suit=random.choice([0, 1, 2, 3]) if game_state.trump_to_be_set else -1,
            )
        ...
        for past_hands in range(round_number):
            ...
            for player_index in range(number_of_players):
                ...
                assert len(game_state.trick_records) == 0
