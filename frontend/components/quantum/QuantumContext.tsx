...
export function QuantumProvider({
  ...
  const setSuitName = (suitName: string) => {
    console.log(canSetSuitName, suitName);
  ...
  useEffect(() => {
    ...
    // Initialize or reinitialize player guess when:
    // 1. No guess data exists yet, OR
    // 2. User has joined a new position (position changed from previous)
    if (Object.keys(playerGuess).length === 0) {
      console.log("Initializing player guess");
      console.log(gameState.current_hands);
      // Use same logic as reset button - just copy current hands safely
  ...
  }, [gameState, currentUserPosition]); // Removed playerGuess to prevent infinite loop
  ...
  useEffect(() => {
    ...
    const newPlayerGuess = structuredClone(playerGuess);
