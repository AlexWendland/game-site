...
import { QuantumHandState, QuantumLogEntry, QuantumActionOutcome } from "./QuantumContext";
...
describe("applyLogToGuess", () => {
  ...
  test("applies target_player log correctly", () => {
    ...
    const result = applyLogToGuess(initialGuess, logs, 0);
    expect(result[1].suits[0]).toBe(1); // No change since already > 0
  });
  ...
  test("applies positive respond_to_target log correctly", () => {
    ...
    const result = applyLogToGuess(initialGuess, logs, 1);
    expect(result[1].suits[0]).toBe(0); // Targeted player loses card
    expect(result[0].suits[0]).toBe(2); // Targeting player gains card
  });
  ...
  test("applies negative respond_to_target log correctly", () => {
    ...
    const result = applyLogToGuess(initialGuess, logs, 1);
    expect(result[1].does_not_have_suit).toContain(0);
    expect(result[1].suits[0]).toBe(0);
  });

  test("skips logs with unsuccessful outcomes", () => {
    const logs: QuantumLogEntry[] = [{
      player: 0,
      function_call: "target_player",
      parameters: { targeted_player: 1, suit: 0 },
    ...
    const result = applyLogToGuess(initialGuess, logs, 0);
    expect(result).toEqual(initialGuess);
  });
  ...
  test("throws error when respond_to_target has no corresponding target log", () => {
    ...
    expect(() => applyLogToGuess(initialGuess, logs, 0)).toThrow("Target log not found");
  });
});
...
describe("canIncreasePlayerGuess", () => {
  ...
  test("returns true when increase is valid", () => {
    ...
    const result = canIncreasePlayerGuess(validPlayerGuess, validCurrentHands, 0, 0);
...
describe("canTogglePlayerDoesNotHaveSuit", () => {
  test("returns false when current hands show player doesn't have suit", () => {
    ...
    const result = canTogglePlayerDoesNotHaveSuit(playerGuess, currentHands, 0, 2);
    expect(result).toBe(false);
  });
  ...
  test("returns false when current hands show player has cards of suit", () => {
    ...
    const result = canTogglePlayerDoesNotHaveSuit(playerGuess, currentHands, 0, 0);
    expect(result).toBe(false);
  });
...
describe("isGuessValid", () => {
  ...
  test("ignores UNKNOWN_SUIT in validation", () => {
    const playerGuess = {
      0: { suits: { 0: 2, 1: 2, "-1": 5 }, does_not_have_suit: [], total_cards: 4 },
      1: { suits: { 0: 2, 1: 2, "-1": 3 }, does_not_have_suit: [], total_cards: 4 },
    };
    const currentHands = {
      0: { suits: { 0: 2, 1: 2, "-1": 5 }, does_not_have_suit: [], total_cards: 4 },
      1: { suits: { 0: 2, 1: 2, "-1": 3 }, does_not_have_suit: [], total_cards: 4 },
...
describe("updatePlayerGuessFromGameState", () => {
  ...
  test("returns full hands for players with full hints", () => {
    ...
    expect(result.updatedGuess).toEqual(safeHandCopy(gameStateWithFullHints.current_hands));
  ...
  test("falls back to current hands when log application fails", () => {
    ...
    expect(result.updatedGuess).toEqual(safeHandCopy(gameStateWithBadLogs.current_hands));
    expect(result.newLogIndex).toBe(1);
  });
});